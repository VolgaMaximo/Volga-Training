...
export async function GET(){
 ...
 const {data,error}=await supabase.from('training_materials').select('id,title,body,sort_order').eq('version_id',version.id).eq('active',true).order('sort_order');
 if(error)return NextResponse.json([],{status:200});
 return NextResponse.json((data||[]).map(x=>({id:x.id,title:x.title,body:x.body||''})));
}
